...
import { RELEASE } from '../download-block/release.data';
...
/**
 * Compact star count. Buckets per ~100: >= 1000 renders as one-decimal "k"
 * (1823 -> "1.8k"), 100..999 rounds to the nearest 100, < 100 stays exact.
 * Returns '' for 0 so the badge can be hidden entirely.
 */
    ...
    >
      <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center gap-3 ml-auto">
          ...
          <!-- GitHub link capsule (real star count baked at build time) -->
          <a
            href="https://github.com/Chidi09/crush"
            target="_blank"
            rel="noopener"
            aria-label="View Crush on GitHub"
...
export class NavComponent {
  ...
  // Real GitHub star count, baked at build time (rate-limit-free). Formatted
  // compactly so it buckets per ~100 (1823 -> "1.8k"); hidden when 0 so we
  // never show an empty/zero badge.
  stars = signal(formatStars(RELEASE.stars));
