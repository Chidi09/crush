...
pub enum RuntimeType {
    Node, TypeScript, Python, Rust, Go, Java, DotNet, Ruby, Php, Elixir, Swift, Deno, Bun,
    Flutter, ReactNative, Generic,
}
...
impl RuntimeType {
    ...
    /// Mobile/app runtimes run on an emulator or device via their own toolchain,
    /// not as an OCI container or a host web server.
    pub fn is_mobile(&self) -> bool {
        matches!(self, RuntimeType::Flutter | RuntimeType::ReactNative)
    }
}
...
impl CrushSpecDetector {
    ...
    pub fn detect(&self, root: &PathBuf) -> Detection {
        ...
        // Mobile/app stacks first — they share manifests with other runtimes
        // (React Native has a package.json), so detect them at confidence 1.0
        // and let max_by pick them over the generic Node/JS detection.
        if let Some(d) = self.try_flutter(root) { detections.push(d); }
        if let Some(d) = self.try_react_native(root) { detections.push(d); }
    ...
    fn resolve_base_image(d: &Detection) -> String {
        ...
        match d.runtime_type {
            ...
            // Mobile stacks build/run via their native toolchain on a device or
            // emulator — there is no OCI base image.
            RuntimeType::Flutter | RuntimeType::ReactNative => String::new(),
            RuntimeType::Generic => {
    ...
    /// Flutter app: a `pubspec.yaml` that depends on the Flutter SDK. (A plain
    /// Dart package has a pubspec but no `sdk: flutter`, so we require that.)
    fn try_flutter(&self, root: &Path) -> Option<Detection> {
        let pubspec = root.join("pubspec.yaml");
        if !pubspec.exists() { return None; }
        let content = fs::read_to_string(&pubspec).ok()?;
        // The flutter SDK dependency is written as `flutter:\n    sdk: flutter`.
        if !content.contains("sdk: flutter") { return None; }
        ...
        // Which target platforms the project supports (folders generated by
        // `flutter create`). Used by the runner to pick a device.
        let mut platforms = Vec::new();
        for p in ["android", "ios", "web", "windows", "macos", "linux"] {
            if root.join(p).is_dir() { platforms.push(p.to_string()); }
        }
        let framework = if platforms.is_empty() {
            "Flutter".to_string()
        } else {
            format!("Flutter [{}]", platforms.join(", "))
        };

        Some(Detection {
            project_name: name,
            runtime_type: RuntimeType::Flutter,
            runtime_version: "stable".to_string(),
            framework_name: framework,
            framework_detected: true,
            // Dev = run on a device/emulator; build = release artifact.
            build_command: "flutter build apk --release".to_string(),
            entry_point: "flutter run".to_string(),
            dev_entry_point: "flutter run".to_string(),
            port: 0, // mobile: no host port (web target picks one at run time)
            confidence: 1.0,
    ...
    /// React Native app: a `package.json` that depends on `react-native`
    /// (bare RN or Expo). Beats `try_node` via confidence 1.0.
    fn try_react_native(&self, root: &Path) -> Option<Detection> {
        let pkg = root.join("package.json");
        if !pkg.exists() { return None; }
        let content = fs::read_to_string(&pkg).ok()?;
        let json: serde_json::Value = serde_json::from_str(&content).ok()?;
        let has_dep = |k: &str| json["dependencies"].get(k).is_some()
            || json["devDependencies"].get(k).is_some();
        if !has_dep("react-native") { return None; }

        let name = json["name"].as_str()
            .filter(|s| !s.is_empty())
            .unwrap_or_else(|| root.file_name().and_then(|n| n.to_str()).unwrap_or("app"))
            .to_string();
        let is_expo = has_dep("expo");
        let framework = if is_expo { "React Native (Expo)" } else { "React Native" };

        // Expo runs through its own CLI; bare RN through the react-native CLI.
        let (dev_entry, build_cmd) = if is_expo {
            ("npx expo start".to_string(), "npx expo prebuild && npx expo run:android".to_string())
        } else {
            ("npx react-native run-android".to_string(), "npx react-native build-android --mode=release".to_string())
        };

        Some(Detection {
            project_name: name,
            runtime_type: RuntimeType::ReactNative,
            runtime_version: "lts".to_string(),
            framework_name: framework.to_string(),
            framework_detected: true,
            build_command: build_cmd,
            entry_point: dev_entry.clone(),
            dev_entry_point: dev_entry,
            dev_install_command: "npm install".to_string(),
            port: 0,
            confidence: 1.0,
            base_image: String::new(),
            ..Default::default()
    ...
    fn try_node(&self, root: &Path) -> Option<Detection> {
        ...
        // React Native projects have a package.json too — let try_react_native
        // own them instead of detecting them as a plain Node web app.
        if json["dependencies"].get("react-native").is_some()
            || json["devDependencies"].get("react-native").is_some() {
            return None;
        }
