<script lang="ts" module>
  ...
  // Inline fallbacks for data services. `simple-icons` renames/removes icons
  // between majors and its named exports can resolve to `undefined` at runtime
  // (Redis showed up as an "R" monogram). These hard-coded Simple Icons paths
  // guarantee the databases/stores the user runs always render a real logo,
  // regardless of the installed simple-icons version. Only used when the MAP
  // entry is missing, so there's no regression when the package resolves fine.
  const FALLBACK: Record<string, SI> = {
    redis: { title: 'Redis', hex: 'FF4438', path: 'M22.71 13.145c-1.66 2.092-3.452 4.483-7.038 4.483-3.203 0-4.397-2.825-4.48-5.12.701 1.484 2.073 2.685 4.214 2.63 4.117-.133 6.94-3.852 6.94-7.239 0-4.05-3.022-6.972-8.268-6.972-3.752 0-8.4 1.428-11.455 3.685C2.59 6.937 3.885 9.958 4.35 9.626c2.648-1.904 4.748-3.13 6.784-3.744C8.12 9.244.886 17.05 0 18.425c.1 1.261 1.66 4.648 2.424 4.648.232 0 .431-.133.664-.365a100.49 100.49 0 0 0 5.54-6.765c.222 3.104 1.748 6.898 6.014 6.898 3.819 0 7.604-2.756 9.33-8.965.2-.764-.73-1.361-1.261-.73zm-4.349-5.013c0 1.959-1.926 2.922-3.685 2.922-.941 0-1.664-.247-2.235-.568 1.051-1.592 2.092-3.225 3.21-4.973 1.972.334 2.71 1.43 2.71 2.619z' },
    ...
    minio: { title: 'MinIO', hex: 'C72E49', path: 'M13.2072.006c-.6216-.0478-1.2.1943-1.6211.582a2.15 2.15 0 0 0-.0938 3.0352l3.4082 3.5507a3.042 3.042 0 0 1-.664 4.6875l-.463.2383V7.2853a15.4198 15.4198 0 0 0-8.0174 10.4862v.0176l6.5487-3.3281v7.621L13.7794 24V13.6817l.8965-.4629a4.4432 4.4432 0 0 0 1.2207-7.0292l-3.371-3.5254a.7489.7489 0 0 1 .037-1.0547.7522.7522 0 0 1 1.0567.0371l.4668.4863-.006.0059 4.0704 4.2441a.0566.0566 0 0 0 .082 0 .06.06 0 0 0 0-.0703l-3.1406-5.1425-.1484.1425.1484-.1445C14.4945.3926 13.8287.0538 13.2072.006Zm-.9024 9.8652v2.9941l-4.1523 2.1484a13.9787 13.9787 0 0 1 2.7676-3.9277 14.1784 14.1784 0 0 1 1.3847-1.2148z' },
  };
  FALLBACK['postgresql'] = FALLBACK['postgres'];
  FALLBACK['mongo'] = FALLBACK['mongodb'];
  FALLBACK['valkey'] = FALLBACK['redis'];

  function norm(name: string): string {
    return name.toLowerCase().replace(/[^a-z0-9.]/g, '');
  }

  // A MAP hit can still be `undefined` when the simple-icons named export didn't
  // resolve, so treat a missing `.path` as "no icon" and fall through to the
  // inline FALLBACK before giving up to a monogram.
  function valid(si: SI | undefined): SI | null {
    return si && si.path ? si : null;
  }

  export function lookupTech(name: string | null | undefined): SI | null {
    if (!name) return null;
    const key = norm(name);
    const k2 = key.replace(/\./g, '');
    return valid(MAP[key]) ?? valid(MAP[k2]) ?? FALLBACK[key] ?? FALLBACK[k2] ?? null;
  }
