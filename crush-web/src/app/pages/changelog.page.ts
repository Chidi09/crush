        ...
        @for (release of releases; track release.version) {
          <div class="border-l-2 border-crush-border/50 pl-6 sm:pl-8 pb-12 relative last:pb-0">
            ...
            <div class="flex items-center gap-4 mb-4 flex-wrap">
              <h2 class="text-2xl font-bold text-white font-mono leading-none">
                {{ release.version }}
              </h2>
              <span
            ...
            <ul class="space-y-3">
              @for (item of release.items; track item) {
                <li
                  class="text-sm sm:text-base text-slate-300 flex items-start gap-2.5 leading-relaxed"
                >
                  <span
                    class="text-crush-orange mt-1.5 shrink-0 w-1.5 h-1.5 rounded-full bg-crush-orange"
                  ></span>
...
export default class ChangelogPage implements OnInit {
  releases = [
    ...
    },
    {
      ...
      items: [
        '<strong>Localhost tunneling:</strong> <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">crush tunnel &lt;port&gt;</code> and <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">crush run --tunnel</code> expose a local port to the public internet for webhooks (Paystack, Stripe, Clerk). Free <strong>cloudflared</strong> quick tunnel by default — no account, no domain, auto-downloaded — with <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">ngrok</code>/<code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">outray</code> as token-gated fallbacks. Crush auto-detects when a project needs a tunnel and the GUI offers a one-click "expose publicly" button.',
        '<strong>Blue-green deployment:</strong> <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">crush deploy --strategy blue-green</code> ships zero-downtime releases on any SSH/VPS host. Crush brings the idle color up beside the live one, health-checks it, then <em>atomically</em> flips a built-in L4 gateway and drains the old release. If the new release fails its health check, nothing flips — the old one keeps serving and the new one is removed.',
        '<strong>Database snapshots (Time Machine):</strong> <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">crush db snapshot &lt;name&gt;</code> / <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">crush db restore &lt;name&gt;</code> freeze and restore exact database state (Postgres &amp; MySQL) by wrapping native <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">pg_dump</code>/<code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">mysqldump</code>. Resolves the connection from <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">DATABASE_URL</code> or a crush-managed service automatically.',
        '<strong>Local mail catcher:</strong> an embedded SMTP sink on <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">:1025</code> captures every email an app sends in development (signup, reset, receipts) instead of delivering it. Crush injects <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">SMTP_HOST</code>/<code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">SMTP_PORT</code> automatically and renders captured mail in a new GUI <strong>Mailbox</strong> tab. CLI: <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">crush mail</code>.',
        '<strong>Cross-OS eject linter:</strong> <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">crush lint</code> (and an automatic pre-flight during <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">crush eject</code>) flags case-sensitive import paths that work on Windows (NTFS) but break in a Linux container — catching "works on my machine" deploy failures before CI. Exits non-zero, so it doubles as a CI gate.',
        '<strong>Managed/BaaS detection:</strong> <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">crush detect</code> and the GUI now surface the external services a project talks to — Supabase, Firebase, Appwrite, Neon, PlanetScale, Turso, Upstash, Clerk, Auth0, Stripe, Paystack, Flutterwave, Sentry, PostHog and more — read from your env files, each with its brand icon.',
        '<strong>Curated image catalog:</strong> <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">crush catalog</code> and a GUI browser list popular images (FlareSolverr, databases, search, storage, observability) you can pull in one click.',
        '<strong>Mobile apps:</strong> Flutter, React Native and Expo are auto-detected and run on a device/emulator, with a live emulator mirror beside the logs in the GUI.',
        '<strong>CI/CD generator:</strong> <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">crush ci</code> writes a stack-aware GitHub Actions, AppVeyor, or Codemagic pipeline (auto-picks Codemagic for mobile).',
        '<strong>Native log streaming &amp; cleanup:</strong> <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">crush deploy --logs -f</code> streams live logs from your cloud provider; <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">crush clean</code> reclaims disk by removing dependency installs, build outputs, and tool caches.',
        '<strong>GUI polish:</strong> docs page scrollspy navigation and roomier spacing, stack-aware image icons, real crushed/exportable images, run persistence across navigation, and a fix for the sled image-store lock that broke <code class="text-crush-orange bg-crush-orange/10 px-1 py-0.5 rounded">eject</code>.',
        '<strong>Reliability:</strong> fixed process-tree kill on failed builds, smarter turborepo/monorepo detection (knows when to run services separately), and polyglot monorepo support (e.g. API on a server, web on Vercel).',
      ],
    },
    {
      version: 'v0.8.1',
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'Restores four output lines that v0.7.73&#x27;s run_project refactor dropped against v0.7.72:',
      items: [
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'Pre-work for the v0.8 GUI implementation. CLI behavior unchanged for normal usage.',
      items: [
        '*New:**',
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'Sets PYTHONUTF8=1 (PEP 540) and PYTHONUNBUFFERED=1 when crush spawns a Python app:',
      items: [
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'Implements PGVECTOR_PLAN.md. When a project asks for pgvector/pgvector:* and Docker isn&#x27;t available, crush now builds the extension against the host PostgreSQL using MSVC and installs it before the app connects.',
      items: [
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'Cosmetic cleanup of the message printed when a python project has no requirements.txt / pyproject.toml. Just a follow-up to v0.7.68.',
      items: [],
    },
    {
      version: 'v0.7.68',
      date: '2026-05-28',
      headline:
        'Found while running crush on project_approval_system_root: Django detection (v0.7.67) succeeded but the build step failed because there&#x27;s no requirements.txt — deps were installed once into .venv and the manifest forgotten.',
      items: [
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'Extends the docker-shape heuristic (v0.7.48+) to Java/Maven/Gradle. When no Dockerfile/compose exists in the repo, crush now treats it as a dev workflow and:',
      items: [
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'springdoc-openapi lazy-inits on first request (~4s) which exceeds the 700ms probe timeout, causing an AsyncRequestNotUsable stack trace. swagger-ui (which we already probe) is the user-facing URL and back-links to api-docs internally.',
      items: [
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'Spring Boot, FastAPI, NestJS, Express, and Fastify now get a narrower probe list (only paths each framework actually exposes) plus the SPA-shell &#x27;/&#x27; fingerprint is skipped for these backends. Cuts the 5-10 stack traces per crush run.',
      items: [
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'v0.7.61&#x27;s CREATE DATABASE step used psql&#x27;s \gexec meta-command via -c, which is treated as part of the SQL string and silently errored as a syntax error. Now uses plain CREATE DATABASE and ignores the &#x27;already exists&#x27; error.',
      items: [],
    },
    {
      version: 'v0.7.61',
      date: '2026-05-28',
      headline:
        'After postgres starts, run an idempotent CREATE/ALTER ROLE block so configured creds win even when the data dir was init&#x27;d with different ones by an earlier crush version.',
      items: [],
    },
    {
      version: 'v0.7.60',
      date: '2026-05-28',
      headline:
        'v0.7.59 wrapped JAVA_HOME path in quotes unconditionally. cmd.exe doubly-escaped the quotes Tokio adds, producing literal \&quot;path\&quot; that fails to resolve. Now: skip quoting entirely when the path is space-free (scoop/sdkman/jenv installs).',
      items: [],
    },
    {
      version: 'v0.7.59',
      date: '2026-05-28',
      headline:
        'Fixes UnsupportedClassVersionError when the project&#x27;s required JDK is newer than the bare &#x27;java&#x27; on PATH. Now spawn_shell rewrites a leading &#x27;java &#x27; to &#x27;$JAVA_HOME/bin/java &#x27; when JAVA_HOME is set — same JDK Maven uses, so class versions match.',
      items: [
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'cmd.exe doesn&#x27;t expand glob patterns, so &#x27;java -jar target/*.jar&#x27; failed with &#x27;Unable to access jarfile target/*.jar&#x27; even after a successful Maven build.',
      items: [
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'Switch Java/Maven build from &#x27;-DskipTests&#x27; (skips only execution) to &#x27;-Dmaven.test.skip=true&#x27; (skips compile AND execution).',
      items: [
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'Multi-service runs already probed /swagger-ui, /v3/api-docs, /healthz, /graphql etc. Single-service runs (a plain backend API) didn&#x27;t.',
      items: [
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'When image is fresh AND node_modules is current, crush no longer asks &#x27;run it now? [Y/n]&#x27; — it just runs. The prompt is friction on warm reruns where you obviously came here to run the app.',
      items: ['Cold runs and first builds still show the prompt for user verification.'],
    },
    {
      version: 'v0.7.54',
      date: '2026-05-28',
      headline:
        'Crush was still running &#x27;pnpm install&#x27; on warm runs even when node_modules was up to date — paying ~10s for pnpm&#x27;s &#x27;Already up to date&#x27; check.',
      items: [
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'Node entry picks dev vs start based on whether the repo ships prod-shape Docker artifacts.',
      items: [
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'feat(proxy): built-in reverse proxy on localhost:8000 for dev/prod URL parity. Routes /api → backend, / → frontend. WebSocket upgrade (HMR) preserved. --no-proxy disables.',
      items: [],
    },
    {
      version: 'v0.7.40',
      date: '2026-05-28',
      headline:
        'feat(windows): Job Object with KILL_ON_JOB_CLOSE — no more orphaned processes when crush exits',
      items: [],
    },
    {
      version: 'v0.7.39',
      date: '2026-05-28',
      headline:
        'fix: tighten compose dep classifier (no more blue/green app services as deps) + bump Garnet to v1.1.9',
      items: [],
    },
    {
      version: 'v0.7.38',
    ...
    },
    {
      ...
      date: '2026-05-28',
      headline:
        'fix: compose discovery in infra/docker subdirs + monorepo prod entry prefers start over dev',
      items: [],
    },
    {
      version: 'v0.7.36',
      date: '2026-05-28',
      headline:
        'fix(probe): drop SPA-shell false positives from service-link probe (no more bogus /swagger-ui on Vite frontends)',
      items: [],
    },
    {
      version: 'v0.7.35',
      date: '2026-05-27',
      headline:
        'feat(cache): skip rebuilds when artifacts are newer than sources (Node/Go/Java). --rebuild forces a build.',
      items: [],
    },
    {
      version: 'v0.7.34',
      date: '2026-05-27',
      headline:
        'feat(urls): scrape http://localhost URLs from child stdout into Ready panel — surfaces secondary listeners and turbo/monorepo child URLs',
      items: [],
    },
    {
      version: 'v0.7.33',
      date: '2026-05-27',
      headline:
        'fix(windows): rewrite ./ prefix in spawn_shell + name go binary .exe so cmd.exe resolves the built artifact correctly',
      items: [],
    },
    {
      version: 'v0.7.32',
      date: '2026-05-27',
      headline: '',
      items: ['*Full Changelog**: https://github.com/Chidi09/crush/compare/v0.7.31...v0.7.32'],
    },
    {
      version: 'v0.7.31',
      date: '2026-05-27',
      headline: '',
      items: ['*Full Changelog**: https://github.com/Chidi09/crush/compare/v0.7.30...v0.7.31'],
    },
    {
      version: 'v0.7.30',
      date: '2026-05-27',
      headline: '',
      items: ['*Full Changelog**: https://github.com/Chidi09/crush/compare/v0.7.29...v0.7.30'],
    },
    {
      version: 'v0.7.29',
      date: '2026-05-27',
      headline: '',
      items: ['*Full Changelog**: https://github.com/Chidi09/crush/compare/v0.7.28...v0.7.29'],
    },
    {
      version: 'v0.7.28',
      date: '2026-05-27',
      headline:
        'fix(multi): bail bind loop when remaining services are dead (Ready panel shows immediately)',
      items: [],
    },
    {
      version: 'v0.7.27',
      date: '2026-05-27',
      headline:
        'feat(ux): Ready panel + swagger/health probes + interactive log filter (a/e/p/N/q)',
      items: [],
    },
    {
      version: 'v0.7.26',
      date: '2026-05-27',
      headline:
        'feat(ux): prefixed multi-service logs; named exit announce; crush eject (Dockerfile + compose)',
      items: [],
    },
    {
      version: 'v0.7.25',
      date: '2026-05-27',
      headline:
        'feat(stacks): runnable entry commands for Rails, Laravel, Phoenix, .NET, Flask; multi-service support for all',
      items: [],
    },
    {
      version: 'v0.7.24',
    ...
    },
    {
      ...
      date: '2026-05-27',
      headline:
        'Patch: fix scanner false positives (pnpm-lock.yaml, test files, admin role labels). crush update will now detect this as a newer version.',
      items: [],
    },
    {
      version: 'v0.5.0',
      date: '2026-05-27',
      headline:
        'Scanner false-positive fixes: pnpm-lock.yaml excluded, test files skip vuln scanner, HARDCODED_ADMIN pattern tightened. Plus instant crush default command and base-image fallbacks.',
      items: [],
    },
    {
      version: 'v0.4.0',
